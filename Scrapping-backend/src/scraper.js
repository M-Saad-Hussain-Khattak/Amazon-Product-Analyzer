...
app.post('/scrape', async (req, res) => {
    ...
    try {
        ...
        const maxDrivers = Math.min(allAsins.length, 6);
        console.log(`Creating ${maxDrivers} browser instances for parallel processing`);
        ...
        const processingPromises = allAsins.map((currentAsin, index) => {
            ...
            return (async () => {
                try {
                    ...
                    const criticalReviews = await getCriticalReviews(currentPage, currentAsin, NUMBER_OF_CRITICAL_REVIEWS);

                    console.log(`\nDriver ${index % drivers.length + 1}: Collecting positive reviews for ASIN: ${currentAsin}`);
                    const positiveReviews = await getPositiveReviews(currentPage, currentAsin, NUMBER_OF_POSITIVE_REVIEWS);
                    ...
                    const productData = {
                        ...
                        review_count: productDescription.reviewCount,
                        description: productDescription.bulletPoints,
                        specifications: productDescription.specifications,
